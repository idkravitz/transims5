...
void ExportNet::Write_Route (void)
{
	...
	for (line_itr = line_array.begin (); line_itr != line_array.end (); line_itr++) {
		...
		if (select_modes && !select_mode [mode]) continue;
		if (select_routes && !route_range.In_Range (route)) continue;
		
		route_rec.Clear ();
		route_rec.Route (route);
		route_rec.Mode (mode);
		route_rec.Veh_Type (line_itr->Type ());
		route_rec.Name (line_itr->Name ());

		route_rec.periods.reserve (num_periods);
