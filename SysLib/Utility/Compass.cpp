...
const char * Compass_Points::point_text [] = {
...
int Compass_Points::Bearing_Point (int bearing)
{
	int point;
...
const char * Compass_Points::Direction_Text (double dx, double dy)
{
...
const char * Compass_Points::Point_Text (int dir, int num_points)
{
	if (compass_points > 0) {
	...
	if (num_points > 0 && num_points < 16) {
		dir = num_points * (dir + 1) / 16;
		if (dir >= num_points) dir = 0;
		dir *= 16 / num_points;
	}
