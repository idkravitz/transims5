...
Arcview_File::Arcview_File (Access_Type access) : 
	Db_Header (access, DBASE), Points (), Projection ()
{
	Initialize ();
}
...
Arcview_File::Arcview_File (string filename, Access_Type access) : 
	Db_Header (access, DBASE), Points (), Projection ()
{
	Initialize ();
...
bool Arcview_File::Open (string filename)
{
	...
	status = Db_Header::Open (filename);
...
bool Arcview_File::Read_Record (int number)
{
	...
	for (;; number++) {
		if (!Db_Header::Read_Record (number)) return (false);
		if (*Record_String () == ' ') break;
	}
